...
import com.ps.user.exception.NoDataFoundException;
...
public class UserServiceImpl implements UserService {
	...
	public Mono<APPUser> createUser(final APPUser user) {
		...
		Mono<APPUser> appUser = userRepository.findUser(user).collectList().flatMap(data -> {
			log.info("Retrived  Data " + data);
			if (data.isEmpty()) {
				return userRepository.save(user);
			} else {
				return Mono.error(new AlreadyExistsException(HttpStatus.CONFLICT,
						"User already exist with same user name or mobile or email "));
			}
		});
	...
	public Mono<APPUser> updateUser(final APPUser user) {
		return userRepository.findById(user.getId()).flatMap(result -> {
			  if(result== null) {
					return Mono.error(new NoDataFoundException(HttpStatus.CONFLICT,	"User does not exist"));
