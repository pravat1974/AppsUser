...
public class PostgressDBConfig extends AbstractR2dbcConfiguration {
	 ...
	 * @Bean
	 * 
	 * @Profile("test") public ConnectionFactory testConnectionFactory() {
	 * System.out.println("USERNAME "+USERNAME);
	 * System.out.println("DBPASSWORD "+DBPASSWORD);
	 * System.out.println("DBDATABASE "+DBDATABASE);
	 * 
	 * ConnectionFactory connectionFactory = ConnectionFactories
	 * .get(ConnectionFactoryOptions .builder() .option(DRIVER, "pool")
	 * .option(PROTOCOL, "postgresql") .option(HOST, DBHOST) .option(PORT, DBPORT)
	 * .option(USER, USERNAME) .option(PASSWORD, DBPASSWORD) .option(DATABASE,
	 * DBDATABASE) .option(MAX_SIZE, 10)
	 * 
	 * .build());
	 * 
	 * 
	 ...
	 * .maxCreateConnectionTime(Duration.ofSeconds(10))
	 * .validationQuery("select 1 from appuser") .build(); return new
	...
	/*
	 * @Profile("test") public R2dbcEntityTemplate getTestReactiveTemplate() {
	 * 
	 ...
	 * @Bean
	 * 
	 * @Profile("test") public ReactiveTransactionManager
	 * testTransactionManager(ConnectionFactory connectionFactory) { return new
	 * R2dbcTransactionManager(testConnectionFactory()); }
	 */
